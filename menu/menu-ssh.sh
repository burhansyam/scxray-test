...
CEKFILE=/etc/alvi/ssh.txt
if [ -f "$CEKFILE" ]; then
file001="OK"
else
touch /etc/xray/ssh.txt
fi
if grep -qw "$Login" /etc/alvi/ssh.txt; then
...
echo "$Login" >> /etc/alvi/ssh.txt
fi
if [ -z $Login ]; then
echo -e "$COLOR1│${NC} [Error] Username cannot be empty "
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1      alvi AIO SC    Service${NC}"
echo ""
read -n 1 -s -r -p "    Press any key to back on menu"
menu-ssh
fi
...
wget -q -O /usr/bin/proxy3.js "https://raw.githubusercontent.com/kyt-team/ws/main/ssh/proxy3.js"
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
wget -q -O /usr/bin/ssh-wsenabler "https://raw.githubusercontent.com/kyt-team/ws/main/ssh/sshws-true.sh" && chmod +x /usr/bin/ssh-wsenabler
...
ssl="$(cat ~/log-install.txt | grep -w "Stunnel4" | cut -d: -f2)"
OhpSSH=`cat /root/log-install.txt | grep -w "OHP SSH" | cut -d: -f2 | awk '{print $1}'`
Login=alvi`</dev/urandom tr -dc X-Z0-9 | head -c4`
hari="1"
Pass=1
...
$COLOR1│$NC   ${COLOR1}[11]${NC} • lock ssh
$COLOR1│$NC   ${COLOR1}[12]${NC} • unlock ssh
...
09 | 9) clear ; ceklim ;;
10 | 10) clear ; delete ;;
01 | 11) clear ; user-lock1 ;;
12 | 12) clear ; user-unlock1 ;;
00 | 0) clear ; menu ;;
*) clear ; menu-ssh ;;
esac
